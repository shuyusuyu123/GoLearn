...
const SERVER_PORT2 = 8080
const SERVER_DOMAIN2 = "localhost"
const RESPONSE_TEMPLATE2 = "hello"

func rootHandler2(w http.ResponseWriter, req *http.Request) {
	w.Header().Set("Content-Type", "text/html")
	w.Header().Set("Content-Length", fmt.Sprint(len(RESPONSE_TEMPLATE2)))
	w.Write([]byte(RESPONSE_TEMPLATE2))
}
...
func main() {
	http.HandleFunc(fmt.Sprintf("%s:%d/", SERVER_DOMAIN2, SERVER_PORT2), rootHandler2)
	YourListenAndServeTLS(fmt.Sprintf(":%d", SERVER_PORT2), "rui.crt", "rui.key", nil)
}
